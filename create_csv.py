...
if __name__ == "__main__":
    ...
    df_1 = create_annotation_file(
        path_folder='./CT_data/interim/emph_pretrain_with_scapis/LDCT',
        domain='LD',
    )
    df_2 = create_annotation_file(
        path_folder='./CT_data/interim/emph_pretrain_with_scapis/HDCT',
        domain='HD',
    )

    """df = create_annotation_file(
         path_folder='./CT_data/interim/autoencoder_val/patients',
        domain="patients",
     )"""

    df = pd.concat([df_1, df_2], axis=0, ignore_index=True)
    df.to_csv('emph_pretrain_with_scapis.csv')
