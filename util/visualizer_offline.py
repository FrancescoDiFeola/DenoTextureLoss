...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        util.mkdirs([self.web_dir, self.img_dir]) if (opt.image_folder is not None) else "pass"
        util.mkdirs([self.web_dir, self.loss_dir]) if (opt.loss_folder is not None) else "pass"
        util.mkdirs([self.web_dir, self.test_dir]) if (opt.test_folder is not None) else "pass"
        util.mkdirs([self.web_dir, self.metric_dir]) if (opt.metric_folder is not None) else "pass"
        ...
        if save_result or not self.saved:  # save images to an HTML file if they haven't been saved.
            ...
            for label, image in visuals.items():
                ...
                if self.opt.isTrain == False:
                    img_path = os.path.join(self.test_dir, 'epoch%.3d_%s.png' % (epoch, label))
    ...
    def plot_current_losses(self, epoch, counter_ratio, current_losses, tracked_losses, name_title):
        """display the current losses on visdom display: dictionary of error labels and values

        Parameters:
            epoch (int)           -- current epoch
            counter_ratio (float) -- progress (percentage) in the current epoch, between 0 to 1
            current losses (OrderedDict)  -- training losses stored in the format of (name, float) pairs
            tracked losses (OrderedDict) -- the whole losses history so far
        """
        if not hasattr(self, 'plot_data'):
            self.plot_data = {'X': [], 'Y': [], 'legend': list(current_losses.keys())}
        self.plot_data['X'].append(epoch + counter_ratio)
        self.plot_data['Y'].append([current_losses[k] for k in self.plot_data['legend']])
    ...
    def plot_metrics(self, tracked_metrics, epoch_performance, epoch, name_title):
        """display the current losses on visdom display: dictionary of error labels and values
               ...
               Parameters:
                   epoch (int)           -- current epoch
                   counter_ratio (float) -- progress (percentage) in the current epoch, between 0 and 1
                   current losses (OrderedDict)  -- training losses stored in the format of (name, float) pairs
                   tracked losses (OrderedDict) -- the whole losses history so far
               """
